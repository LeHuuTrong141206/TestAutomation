...
public class VideoTest_TTND_02 {
    WebDriver driver;
    WebDriverWait wait;
    ...
    public void testTTND_01_ThichVideoThanhCong() {
        ...
        By likeButtonLocator =
                By.xpath("//input[@name='videoId' and @value='20']/following-sibling::button");

        WebElement btnYeuThich =
                wait.until(ExpectedConditions.elementToBeClickable(likeButtonLocator));

        btnYeuThich.click();

        // Đợi trạng thái nút đổi text
        wait.until(ExpectedConditions.textToBe(likeButtonLocator, "Yêu thích"));


        driver.get("http://localhost:8080/cinevo-web/cinevo/user?tab=favorite");

        By videoLocator =
                By.xpath("//a[contains(@href,'tab=video-detail') and contains(@href,'id=20')]");

        WebElement likedVideo =
                wait.until(ExpectedConditions.visibilityOfElementLocated(videoLocator));
